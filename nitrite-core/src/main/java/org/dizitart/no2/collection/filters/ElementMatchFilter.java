...
class ElementMatchFilter implements Filter {
    ...
    ElementMatchFilter(Field field, Filter elementFilter) {
        this.field = field;
    ...
    public ReadableStream<NitriteId> apply(NitriteMap<NitriteId, Document> documentMap) {
    ...
    private boolean matchEqual(Object item, Filter filter) {
        ...
        if (item instanceof Document) {
            ...
            Object docValue = document.get(((EqualsFilter) filter).getField().getName());
    ...
    private boolean matchGreater(Object item, Filter filter) {
        ...
        if (item instanceof Number && comparable instanceof Number) {
            ...
            Document document = (Document) item;
            Object docValue = document.get(((GreaterThanFilter) filter).getField().getName());
    ...
    private boolean matchGreaterEqual(Object item, Filter filter) {
        ...
        if (item instanceof Number && comparable instanceof Number) {
            ...
            Document document = (Document) item;
            Object docValue = document.get(((GreaterEqualFilter) filter).getField().getName());
            if (docValue instanceof Comparable) {
                Comparable arg = (Comparable) docValue;
                return arg.compareTo(comparable) >= 0;
            } else {
    ...
    private boolean matchLesserEqual(Object item, Filter filter) {
        ...
        if (item instanceof Number && comparable instanceof Number) {
            ...
            Document document = (Document) item;
            Object docValue = document.get(((LesserEqualFilter) filter).getField().getName());
            if (docValue instanceof Comparable) {
                Comparable arg = (Comparable) docValue;
                return arg.compareTo(comparable) <= 0;
            } else {
    ...
    private boolean matchLesser(Object item, Filter filter) {
        ...
        if (item instanceof Number && comparable instanceof Number) {
            ...
            Document document = (Document) item;
            Object docValue = document.get(((LesserThanFilter) filter).getField().getName());
            if (docValue instanceof Comparable) {
                Comparable arg = (Comparable) docValue;
                return arg.compareTo(comparable) < 0;
            } else {
    ...
    private boolean matchIn(Object item, Filter filter) {
        ...
        if (values != null) {
            if (item instanceof Document) {
                Document document = (Document) item;
                Object docValue = document.get(((InFilter) filter).getField().getName());
                if (docValue instanceof Comparable) {
    ...
    private boolean matchNotIn(Object item, Filter filter) {
        ...
        if (values != null) {
            if (item instanceof Document) {
                Document document = (Document) item;
                Object docValue = document.get(((NotInFilter) filter).getField().getName());
                if (docValue instanceof Comparable) {
                    return !values.contains(docValue);
                }
            } else if (item instanceof Comparable) {
    ...
    private boolean matchRegex(Object item, Filter filter) {
        ...
        if (item instanceof String) {
            ...
            Document document = (Document) item;
            Object docValue = document.get(((RegexFilter) filter).getField().getName());
